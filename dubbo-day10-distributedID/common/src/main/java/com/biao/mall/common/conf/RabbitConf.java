...
private final Logger logger = LoggerFactory.getLogger(RabbitConf.class);
    ...
    public static final String DLX_ROUTING_KEY_A2A = "mall.routingKey_A2A";

    public static final String DLX_ROUTING_KEY_DLX2DLX = "mall.routingKey_D2D";
    ...
    public Queue queueA(){
        ...
        args.put("x-dead-letter-routing-key", DLX_ROUTING_KEY_DLX2DLX);
        //60秒
    ...
    public Binding bindingDLQueue(){
        return BindingBuilder.bind(dlqQueue()).to(dlxExchange()).with(DLX_ROUTING_KEY_DLX2DLX);
    }

    //绑定exchangeA --> queueA
    @Bean
    public Binding binding(){
        return BindingBuilder.bind(queueA()).to(directExchange()).with(RabbitConf.DLX_ROUTING_KEY_A2A);
    }
