...
private final Logger logger = LoggerFactory.getLogger(RabbitConf.class);
    ...
    public static final String QUEUE_C = "mall.queue_C";

    public static final String X_ROUTING_KEY_A2A = "mall.routingKey_A2A";

    public static final String X_ROUTING_KEY_A2C = "mall.routingKey_A2C";

    public static final String DLX_ROUTING_KEY_DLX2DLQ = "mall.routingKey_D2D";
    ...
    public Queue queueA(){
        ...
        args.put("x-dead-letter-routing-key", DLX_ROUTING_KEY_DLX2DLQ);
        //5分钟
    ...
    //绑定DLXexchangeB --> DLXqueue
    @Bean
    public Binding bindingDLQueue(){
        return BindingBuilder.bind(dlqQueue()).to(dlxExchange()).with(DLX_ROUTING_KEY_DLX2DLQ);
    }

    //绑定exchangeA --> queueA
    @Bean
    public Binding bindingA2A(){
        return BindingBuilder.bind(queueA()).to(directExchange()).with(RabbitConf.X_ROUTING_KEY_A2A);
    }

    //绑定exchangeA --> queueC
    @Bean
    public Binding bindingA2C(){
        return BindingBuilder.bind(QueueC()).to(directExchange()).with(RabbitConf.X_ROUTING_KEY_A2C);
    }
