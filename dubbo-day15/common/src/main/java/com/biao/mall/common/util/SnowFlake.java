...
public class SnowFlake{
    ...
    private final long MAX_WORK_ID = ~(-1L << workerIdBits);
    /** 支持的最大数据标识id，结果是31 */
    private final long MAX_DATA_CENTER_ID = ~(-1L << dataCenterIdBits);
    /** 序列在id中占的位数 */
    ...
    public SnowFlake(long workerId, long dataCenterId) {
        if (workerId > MAX_WORK_ID || workerId < 0) {
            throw new IllegalArgumentException(String.format("workerId" +
                    " can't be greater than %d or less than 0", MAX_WORK_ID));
        }
        if (dataCenterId > MAX_DATA_CENTER_ID || dataCenterId < 0) {
            throw new IllegalArgumentException(String.format("dataCenterId" +
                    " can't be greater than %d or less than 0", MAX_DATA_CENTER_ID));
        }
